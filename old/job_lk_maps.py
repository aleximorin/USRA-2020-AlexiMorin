...
if __name__ == '__main__':
    ...
    job = {'name': 'Job glacier',
           ...
           'full_glate_path': glate_path + '/job_ez_dem_fullGlate.tif',
    ...
    lk = {'name': 'Little Kluane glacier',
          ...
          'full_glate_path': glate_path + '/lk_fullGlate.tif',
          'points': 'Ground penetrating radar\n(Flowers, 2019)',
          'outline': 'Glacier outline\n(Nolan, 2020)',
          'leg_pos': 0.075}
    ...
    for data in datasets:
       ...
       # projection = outline.crs
        ...
        glathida_errors.append(error_array)

        # Computes the relative error
        rel_error = 100 * error / true_thickness_im
        glathida_rel_errors.append(rel_error[np.logical_not(np.isnan(rel_error))])
        ...
        # Computes the difference between glathida and glate
        full_glate_ras = rasterio.open(data['full_glate_path'])
        full_glate_im = full_glate_ras.read(1)
        glathida_glate_diff_im = mod_thickness_im - full_glate_im

        cropped_glate_im, cropped_glate_meta = es.crop_image(full_glate_ras, points)
        cropped_glate_meta.update({"driver": "GTiff",
                                       "height": cropped_glate_im.shape[1],
                                       "width": cropped_glate_im.shape[2],
                                       "transform": cropped_glate_meta["transform"]})
        # Computes the error between the glate and observed data
        glate_error = cropped_glate_im - true_thickness_im
        ...
        # Bounds
        axs[1].set_xlim(c_xmin, c_xmax)
        axs[1].set_ylim(c_ymin, c_ymax)
        ...
        handles.append(mlines.Line2D([], [], color='black', label=data['outline']))
        handles.append(matplotlib.patches.Patch(color='none', label=crs))
        axs[0].legend(handles=handles, bbox_to_anchor=(0.5, data['leg_pos']), loc="lower center",
                      bbox_transform=fig.transFigure, ncol=len(handles), frameon=True, fontsize=12)
        ...
        fig = plt.figure(figsize=(8, 8))
        ax = plt.gca()
        ax.set_title('Error, Farinotti et al. model' + data['name'])
        ...
        ax.legend(handles=handles, bbox_to_anchor=(0.5, data['leg_pos'] / 2), loc="lower center",
                  bbox_transform=fig.transFigure, ncol=len(handles), frameon=True)
        fig.tight_layout()
        fig.savefig(f'{img_folder}/{data["name"]}_glathida_error.png')

        ########################
        # Full Glate error map #
        ########################

        fig = plt.figure(figsize=(8, 8))
        ax = plt.gca()
        ax.set_title('Error, full GlaTe model' + data['name'])

        # Plots the error and adds a colorbar
        img = ax.imshow(glate_error[0], cmap='jet', extent=cropped_extent)
        divider = make_axes_locatable(ax)
        cax = divider.append_axes('right', size=0.2, pad=0.1)
        cbar = fig.colorbar(img, cax=cax, orientation='vertical')
        cbar.ax.get_yaxis().labelpad = 15
        ...
        ###############################
        # Glathida relative error map #
        ###############################

        fig = plt.figure(figsize=(8, 8))
        ax = plt.gca()
        ax.set_title('Relative error, Farinotti et al. model' + data['name'])
    ...
    ax.boxplot(glathida_errors, labels=[f'Job glacier\nN = {len(glathida_errors[0])}',
                               f'Little Kluane\nN = {len(glathida_errors[1])}'])
    ...
    ax.set_ylabel('Error (%)')
    ax.yaxis.grid(True, linestyle='-', which='major', color='lightgrey', alpha=0.5)
    ax.boxplot(glathida_rel_errors, labels=[f'Job glacier\nN = {len(glathida_errors[0])}',
                                   f'Little Kluane\nN = {len(glathida_errors[1])}'])
    fig.savefig(f'{img_folder}/rel_boxplots.png')

    with open(f'{img_folder}/data.csv', 'w') as txtfile:
        cols = 'Mean,Std,Median,Maximum,Minimum,Absolute mean,Mean square,r2'
        txtfile.write('Glacier,' + cols + '\n')
        for i in range(len(datasets)):
            txtfile.write(f"{datasets[i]['name']},")
            table = error_table(glathida_errors[i])
            for num in table:
                txtfile.write(f'{num:2.4},')
            txtfile.write('\n')
            txtfile.write(f"{datasets[i]['name']} (\%),")
            table = error_table(glathida_rel_errors[i])
            for num in table:
                txtfile.write(f'{num:2.4},')
            txtfile.write('\n')
